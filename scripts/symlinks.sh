...
for config in $config_files; do
    mv $config_dir/$config $old_dir/.config 2> /dev/null
    ln -s $dir/$config $config_dir 2> /dev/null
    echo "Linked $config"
done
echo "done..."
...
for binary in $(ls $dir/.local/bin); do
    mv $user_bin/$binary $old_dir/.local/bin/ 2> /dev/null
    ln -s $dir/.local/bin/$binary $user_bin/$binary 2> /dev/null
    echo "Linked $binary"
done
echo "done..."
